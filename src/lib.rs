...
pub mod rule {
        ...
        E: Expression,
    {
        ...
        #[inline]
        pub fn new(top: &'e E::Group, bot: &'e E::Group) -> Self {
            Self { top, bot }
        }
    }
        ...
        E::Group: FromIterator<E> + IntoIterator<Item = E>,
    {
        #[inline]
        fn cases(&self) -> Reference<E> {
            Reference::new(&self.top, &self.bot)
        }
    }
}
    ...
    Self: Into<RatioPair<V>>,
{
        ...
        F: FnMut(&V::Item, &V::Item) -> bool,
    {
        let top = top.into();
        let bot = bot.into();
        let (lower, upper) = util::multiset_symmetric_difference_by::<_, _, V, _>(
            top.bot,
            bot.top.into_iter().collect(),
            eq,
        );
